...
node* getPivot(node* root){
    ...
    node* leftPivot = getPivot(root->leftChild);
    if(leftPivot != nullptr) return leftPivot;
...
void ll_rotation(node* pivot){
    ...
    pivot->leftChild = nullptr;pivot->parent = nullptr;//all pivot's ties have been severed
...
void rr_rotation(node* pivot){
    ...
    node* stupidNode = pivotRight->leftChild;
    ...
    pivot->rightChild = nullptr;pivot->parent = nullptr;//all pivot's ties have been severed
...
void addElement(int data){
    ...
    if(pivot != nullptr){
        ...
        if (rotation == 1){ll_rotation(pivot);}
        else if (rotation == 2){lr_rotation(pivot);}
        else if (rotation == 3){rl_rotation(pivot);}
        else{rr_rotation(pivot);}
    }
    node* newNode = new node(data);
    if(rootNode == nullptr){
        first = rootNode = newNode;
    }else{
        node* parent;
...
int main(){
    ...
    int elements[] = {23,45,6,7,54,34,456,43,21,53,78,10,5};
    for(int i:elements)addElement(i);

    inorderTravel(rootNode);
    std::cout<<whichRotation(getPivot(rootNode))<<std::endl;
    std::cout<<rootNode->data<<std::endl;
    std::cout<<first->parent->data;
